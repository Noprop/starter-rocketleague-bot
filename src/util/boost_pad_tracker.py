...
class BoostPadTracker:
    ...
    def initialize_boosts(self, game_info: FieldInfoPacket):
        raw_boosts = [game_info.boost_pads[i]
                      for i in range(game_info.num_boosts)]
        self.boost_pads: List[BoostPad] = [
            BoostPad(Vec3(rb.location), rb.is_full_boost, False, 0) for rb in raw_boosts]
